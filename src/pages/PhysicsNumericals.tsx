    ...
    <div className="flex h-screen bg-[#ffffff] text-[#0f172a] font-sans overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0f172a] text-white transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
        <div className="p-6 border-b border-white/10">
          ...
          <h1 className="font-serif text-xl text-white mb-1">Physics Bank</h1>
          <p className="font-mono text-[9px] uppercase tracking-widest text-white/40">SH 401 · TU IOE</p>
        </div>
        
        <nav className="p-2 overflow-y-auto h-[calc(100vh-140px)]">
          <div className="px-4 py-2 text-[9px] uppercase tracking-widest text-white/30 font-mono">Curriculum</div>
          <button 
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-10 pb-24">
          {filteredTopics.map((topic, i) => (
            <motion.div 
              ...
              className="mb-12"
            >
              <div className="flex flex-col md:flex-row md:items-center gap-4 border-b border-gray-100 pb-4 mb-6">
                ...
                <div className="flex items-center gap-2 md:ml-auto">
                  <button 
                    ...
                    className={`px-3 py-1 text-xs font-medium rounded-full transition-colors ${activeTab === 'tutorial' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:bg-gray-50'}`}
                  >
                    Tutorial
                  </button>
                  <button 
                    onClick={() => setActiveTab('assignment')}
                    className={`px-3 py-1 text-xs font-medium rounded-full transition-colors ${activeTab === 'assignment' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:bg-gray-50'}`}
                  >
                    Assignment
                  </button>
              ...
              <div className="grid gap-0 border-t border-gray-100">
                {topic.questions.length === 0 ? (
                  <div className="py-8 text-center text-sm text-gray-400 italic">No questions in this section yet.</div>
                ) : (
                  topic.questions.map((q, idx) => (
                    <div 
                      ...
                      className="group grid grid-cols-[60px_1fr_auto] gap-4 py-4 px-4 border-b border-gray-100 hover:bg-gray-50 cursor-pointer transition-colors items-start"
                    >
                      <div className="text-xs font-mono text-gray-400 pt-1">{q.n}</div>
                      <div className="text-[15px] text-gray-800 leading-relaxed group-hover:text-black">
                        {q.t}
                      </div>
                      <div className="text-right">
                        {q.a && q.a !== 'See solution' && q.a !== 'Derivation' ? (
                          <span className="inline-block px-2 py-1 bg-gray-100 text-gray-600 text-[10px] font-mono rounded border border-gray-200 max-w-[150px] truncate">
                            {q.a}
                          </span>
                        ) : (
            ...
            >
              ...
              <div className="px-8 py-6 border-b border-gray-100 flex items-start justify-between bg-white z-10">
                <div>
                  ...
                  <h3 className="text-lg font-medium text-gray-900 leading-relaxed pr-8">
                  ...
                  {selectedQuestion.q.a && selectedQuestion.q.a !== 'See solution' && (
                    <div className="mt-3 flex items-center gap-2">
                      ...
                      <span className="text-sm font-mono text-gray-700 bg-gray-100 px-2 py-0.5 rounded">{selectedQuestion.q.a}</span>
                    </div>
                  )}
                </div>
                <button 
