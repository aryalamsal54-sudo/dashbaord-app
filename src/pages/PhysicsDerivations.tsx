    ...
    <div className="flex h-screen bg-[#ffffff] text-[#0f172a] font-sans overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0f172a] text-white transform transition-transform duration-300 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
        <div className="p-6 border-b border-white/10">
          ...
          <h1 className="font-serif text-xl text-white mb-1">Derivations Bank</h1>
          <p className="font-mono text-[9px] uppercase tracking-widest text-white/40">ENSH 102 · TU IOE</p>
        </div>
            ...
            >
              ...
              <div className="grid gap-3">
                {topic.derivations.map((q, idx) => (
                  ...
                  >
                    <div className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-xs font-serif text-gray-400 group-hover:bg-white group-hover:text-indigo-500 group-hover:border-indigo-200 transition-colors shrink-0 mt-0.5">
                      {idx + 1}
                    </div>
                    <p className="text-[15px] text-gray-700 leading-relaxed group-hover:text-gray-900">
                      {q.t}
                    </p>
                  </div>
                ))}
