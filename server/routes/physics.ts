...
const SOLVER_PROMPT = `You are an expert physicist. Solve the given problem 
...
physics solution and reformat it into clean LaTeX.
...
const EXPLAINER_PROMPT = `You are a friendly physics tutor explaining a solution 
out loud to a confused student. You are given a LaTeX physics solution.
...
router.get('/solution/:id', async (req, res) => {
  const { rows } = await db.query('SELECT * FROM physics_solutions WHERE question_id = $1', [req.params.id]);
  res.json(rows[0] || { solution: null });
});

router.get('/solved-ids', async (req, res) => {
  const { rows } = await db.query('SELECT question_id FROM physics_solutions WHERE solved = 1');
...
router.post('/solve', async (req, res) => {
  ...
  if (!forceRefresh) {
    const { rows } = await db.query('SELECT * FROM physics_solutions WHERE question_id = $1', [questionId]);
    const cached = rows[0];
  ...
  try {
    ...
    `, [questionId, question, JSON.stringify({ formattedSolution: formatted, voiceExplanation: explanation }), 'Groq (3-Phase Pipeline)', topic || 'unknown', tab || 'unknown', 'Physics', '0']);
